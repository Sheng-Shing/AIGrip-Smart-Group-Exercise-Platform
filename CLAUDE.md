...
- **Top-level helpers** (lines ~50–110): `getEntityRole`, `getEntityLayout`, `getEntitySector`, `getLayoutX`, `getMixedPawnY`, `lerp`. `getLayoutX` is **player-count-aware**: 1–4 players all use the same rule — divide screen into `player_count` equal columns by `sector` (p1 leftmost, pN rightmost), `layout` offsets ±30% column width within each column. So a player's left and right hand entities (same `sector`, `layout: left/right`) always land at distinct X within their own column. `sector: "shared"` falls back to legacy 25%/50%/75%.
