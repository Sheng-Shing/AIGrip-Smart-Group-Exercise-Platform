...
- **Top-level helpers** (lines ~50–110): `getEntityRole`, `getEntityLayout`, `getEntitySector`, `getLayoutX`, `getMixedPawnY`, `lerp`. `getLayoutX` is **player-count-aware**: 1–2 players use legacy 25%/50%/75% layout columns; 3–4 players divide screen into N equal columns by `sector` with `layout` offsetting ±30% within each column.
...
- **Layout limitations are real**: only 3 columns for 1–2 players, N columns for 3–4 players, no top/bottom row split, no 2×2 grid, no random pop-up spawn. Section 四 of system_instruction lists what the engine can and can't do — keep it in sync when adding features.
